        ...
        try {
            ...
            dataBody.forEach(async (cate) => {
                cate.body.forEach(async (item) => {
                    ...
                    const phone = item?.contact?.content?.find(i => i.name === "Điện thoại:")?.content
                    users?.every(item => item?.phone !== phone) && users.push({
                        id: userId,
                        name: item?.contact?.content?.find(i => i.name === "Liên hệ:")?.content,
                        password: hashPassword('123456789'),
                        fbUrl: 'https://www.facebook.com/anony',
                        role: 'user',
                    ...
                    await db.Post.create({
                        ...
                        userId: users.find(p => p.phone === item?.contact?.content?.find(i => i.name === "Điện thoại:")?.content)?.id,
                        imageId: imagesId,
